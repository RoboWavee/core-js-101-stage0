...
function findFirstSingleChar(str) {
  return (
    str
      .split('')
      .find((char) => str.split('').filter((c) => c === char).length === 1) ||
    null
  );
}
...
function isCreditCardNumber(ccn) {
  const digits = ccn.toString().replace(/\D/g, '').split('').map(Number);
  let sum = 0;
...
function isBracketsBalanced(str) {
  ...
  let currentStr = str;
  do {
    previousLength = currentStr.length;
    currentStr = currentStr.replace(/\[\]|\(\)|\{\}|<>/g, '');
  } while (currentStr.length !== previousLength);
...
function getCommonDirectoryPath(pathes) {
  ...
  for (let i = 0; i < pathArrays[0].length; i += 1) {
    ...
    if (pathArrays.every((path) => path[i] === part)) {
...
function getMatrixProduct(m1, m2) {
  ...
  for (let i = 0; i < aRows; i += 1) {
    product[i] = new Array(bCols).fill(0);
  }
